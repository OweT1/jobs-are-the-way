...
class JobResultsRepository(BaseRepository):
    def __init__(self, db: PostgresDB):
        super().__init__(db=db, table=JobResults)
    ...
    @db_retry_decorator
    def _check_job_existence_by_id(self, job_id: str) -> bool:
        with self.db.session() as session:
            stmt = select(exists().where(JobResults.job_id == job_id))
            existence: bool = session.scalar(stmt)
        return existence

    async def check_jobs_existence(self, jobs_df: pd.DataFrame) -> pd.DataFrame:
        tasks = [
            asyncio.to_thread(self._check_job_existence_by_id, job_id)
